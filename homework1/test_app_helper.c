 ...
 * test_app_helper.c
 *
 * Sample user-space program to benchmark a custom Linux system call ("app_helper").
 *
 * This code:
 *   1. Allocates a user buffer (size provided as a command-line argument, default is 256 bytes)
 *      and initializes it.
 *   2. Invokes the "app_helper" system call multiple times.
 *   3. Times each invocation and computes average latency.
 *   4. Verifies that the kernel modifies the buffer as expected (e.g., to all 1s).
 *
 * Compilation example (assuming syscall number is defined):
 *   gcc -o test_app_helper test_app_helper.c
 *   ./test_app_helper [BUF_SIZE]
 *
 * NOTE: Replace SYS_app_helper (449) with the actual syscall number you assigned
